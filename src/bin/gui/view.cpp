...
{
public:
  FrameEvent(const Pointer<Frame>& frame) : QEvent(QEvent::User), frame(frame){};
...
public:
  Window(QWidget* parent = 0, Qt::WindowFlags flags = 0)
    : QMainWindow(parent, flags), label(this), statusBar(this), frameId(0), scale(1.0), close(false), blur(false)
  {
  ...
  bool event(QEvent* event)
  {
    if (event->type() == QEvent::User)
    {
      ...
      if (frameEvent->getFrame()->getType() == Frame::typeNormal)
      {
        ...
        label.setPixmap(QPixmap::fromImage(image.scaled(image.size() * scale)));
        setFixedSize(label.pixmap()->width(), label.pixmap()->height() + statusBar.height());
...
public:
  Worker(const Pointer<Device>& device, QWidget* parent = 0)
    : QThread(parent), device(device), interruptRequested(false)
  {
  }
...
{
  Command::Application application("Capture frames from a Seek Thermal camera device");
  application[0] = Command::Argument("Interface address of connected device", "ADDRESS");
  application["scale"] = Command::Argument("Scale of visualized frame image", "SCALE", "1.0");
  application["close"] = Command::Argument("Morphologically close low-variance values", "", "false");
  application["blur"] = Command::Argument("Blur the visualized frame image", "", false);
  application["calibration"] = Command::Argument("Use the specified calibration file", "FILE", "");
  application["device"] = Command::Argument("Type of connected device", "TYPE");
  application["timeout"] = Command::Argument("Request timeout in ms", "TIMEOUT", "1000");

  if (application.parseArguments(argc, argv))
  {
    SeekThermal::Usb::Context context;
    Pointer<Interface> interface = context.getInterface(application[0].getValue());
    ...
    if (!device.isNull())
    {
      interface->setTimeout(application["timeout"].getValue<size_t>() * 1e-3);
